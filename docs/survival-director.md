...
6. **Milestone 7 (Co-op Scaling)** - implemented. `CombatDirectorUtility.ResolveBudgetMultiplier` scales `phase.BudgetPerPulse` by a run curve (`CurveChannel.DirectorBudget`) and a co-op multiplier (`CoopGlobalKey.DirectorBudget`); `CoopGlobalKey.DirectorPressure` scales `SurvivalPhase.TargetPressure` (`PlayerClusterDirectorUtility.BuildAnchors`), `MaxAliveEnemies` and `DirectorConfig.MaxPurchasesPerPulse` (`CombatDirectorUtility.TryPulse`). Both rows live on the `BalanceConfig` asset (see `docs/run-curves-coop-scaling.md`, incl. why the caps needed their own row) - a missing `BalanceConfig` degrades every one of them to a `1x` no-op rather than halting the Director.
