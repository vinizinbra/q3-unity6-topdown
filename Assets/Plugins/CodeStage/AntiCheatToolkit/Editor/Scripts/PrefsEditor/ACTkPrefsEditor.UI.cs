...
namespace CodeStage.AntiCheat.EditorCode
{
	...
	internal partial class ACTkPrefsEditor
	{
		...
		internal static void ShowWindow()
		{
			...
			myself.minSize = new Vector2(500, 0);
			myself.RefreshData();
		}
		...
		private void DrawToolbar()
		{
			using (GUITools.Horizontal(GUITools.Toolbar))
			{
				...
				// Create console-style toggle button
				var toggleRect = GUILayoutUtility.GetRect(20, EditorGUIUtility.singleLineHeight, GUILayout.Width(40));
				var newState = EditorGUI.Toggle(toggleRect, showUnityPrefs, EditorStyles.toolbarButton);
				if (newState != showUnityPrefs)
				{
					ACTkEditorPrefsSettings.ShowUnityPrefs = newState;
					ApplyFiltering();
				}
				
				// Draw Unity icon overlay on the toggle button
				var iconSize = 16f;
				var iconRect = new Rect(toggleRect.x + (toggleRect.width - iconSize) / 2, toggleRect.y + (toggleRect.height - iconSize) / 2, iconSize, iconSize);
				
				var originalColor = GUI.color;
				GUI.color = new Color(1f, 1f, 1f, 0.9f);
				GUI.DrawTexture(iconRect, CSEditorIcons.Unity);
				GUI.color = originalColor;
				
				// Add tooltip
				if (GUI.Button(toggleRect, GUIContent.none, GUIStyle.none))
				{
					// Tooltip is handled by the toggle above
				}
				GUI.Label(toggleRect, new GUIContent("", showUnityPrefs ? "Hide Unity-managed prefs" : "Show Unity-managed prefs"));
			}
		}
		...
		private void DrawRecord(int recordIndex, out bool recordRemoved)
		{
			...
			var guiColor = GUI.color;
			var isUnityManaged = IsUnityManagedPref(record);
			...
			using (GUITools.Horizontal())
			{
				...
				if (record.Obscured)
					GUI.color = obscuredColor;
				else if (IsUnityManagedPref(record))
					GUI.color = UnityManagedColor;
			...
			{
				GUI.enabled = record.prefType != PrefsRecord.PrefsType.Unknown && !isUnityManaged;
				if (record.Obscured)
				{
					...
					if (GUILayout.Button(new GUIContent("D", isUnityManaged ? "Unity-managed pref (cannot decrypt)" : "Decrypt this pref using ObscuredPrefs"), GUITools.CompactButton, GUILayout.Width(25)))
					{
						record.Decrypt();
						GUIUtility.keyboardControl = 0;
						UpdateUnsavedChangesState();
				...
				{
					if (GUILayout.Button(new GUIContent("E", isUnityManaged ? "Unity-managed pref (cannot encrypt)" : "Encrypt this pref using ObscuredPrefs"), GUITools.CompactButton, GUILayout.Width(25)))
					{
						record.Encrypt();
						GUIUtility.keyboardControl = 0;
						UpdateUnsavedChangesState();
			...
			void DrawKey()
			{
				...
				{
					GUI.enabled = false;
					EditorGUILayout.TextField(record.Key, GUILayout.MinWidth(50));
					GUI.enabled = record.prefType != PrefsRecord.PrefsType.Unknown;
				}
				else
				{
					EditorGUI.BeginChangeCheck();
					record.Key = EditorGUILayout.TextField(record.Key, GUILayout.MinWidth(50));
					if (EditorGUI.EndChangeCheck())
					{
						UpdateUnsavedChangesState();
					}
				...
				// Draw Unity icon overlay for Unity-managed prefs
				if (isUnityManaged)
				{
					var lastRect = GUILayoutUtility.GetLastRect();
					var iconSize = 16f;
					var iconRect = new Rect(lastRect.xMax - iconSize - 2, lastRect.y + (lastRect.height - iconSize) / 2, iconSize, iconSize);
					
					var originalColor = GUI.color;
					GUI.color = new Color(1f, 1f, 1f, 0.8f); // Semi-transparent
					GUI.DrawTexture(iconRect, CSEditorIcons.Unity);
					GUI.color = originalColor;
				}
			}
				...
				case PrefsRecord.PrefsType.String:
					if (!record.Obscured || record.IsEditableObscuredValue())
					{
						menu.AddItem(new GUIContent("Paste string value from clipboard"), false, () =>
						{
							...
							instance.UpdateUnsavedChangesState();
						});
					}
					break;
