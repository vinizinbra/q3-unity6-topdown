...
namespace CodeStage.AntiCheat.Detectors
{
	...
	public class WallHackDetector : ACTkDetectorBase<WallHackDetector>
	{
		...
		private const float ValidPlayerPositionZEdge = 0.5f;
		...
		public Vector3 spawnPosition = new Vector3(-10000, -10000, -10000);
		...
		private void FixedUpdate()
		{
			...
			if (rigidPlayer.transform.localPosition.z > ValidPlayerPositionZEdge)
			{
				rigidbodyDetections++;
				if (!Detect())
				{
		...
		private void Update()
		{
			...
			if (charControllerVelocity > 0 && charControllerPlayer.enabled)
			{
				charControllerPlayer.Move(new Vector3(UnityEngine.Random.Range(-0.002f, 0.002f), 0, charControllerVelocity));

				if (charControllerPlayer.transform.localPosition.z > ValidPlayerPositionZEdge)
				{
		...
		private void UpdateServiceContainer()
		{
			if (enabled && gameObject.activeSelf)
			{
				...
				if ((checkRigidbody || checkController) && solidWall == null)
				{
					...
					solidWall.transform.localScale = new Vector3(3, 3, 1f);
					solidWall.transform.localPosition = new Vector3(0, 0, 0.5f);
				}
		...
		private void StartRigidModule()
		{
			...
			if (rigidPlayer.transform.localPosition.z <= ValidPlayerPositionZEdge && rigidbodyDetections > 0)
			{
		...
		private void StartControllerModule()
		{
			...
			if (charControllerPlayer.transform.localPosition.z <= ValidPlayerPositionZEdge && controllerDetections > 0)
			{
#if ACTK_DEBUG_ENABLED
		...
		{
			charControllerPlayer.enabled = false;
			charControllerPlayer.transform.localPosition = new Vector3(-0.75f, 0, -1f);
			charControllerPlayer.enabled = true;
		}
		...
		private void InitControllerModule()
		{
			...
			charControllerPlayer.skinWidth = 0.1f;
			charControllerPlayer.minMoveDistance = 0f;
		}
