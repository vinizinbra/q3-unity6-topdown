...
namespace CodeStage.AntiCheat.Detectors
{
	...
	public class TimeCheatingDetector : ACTkDetectorBase<TimeCheatingDetector>
	{
...
#if ACTK_AWAITABLE
		/// <summary>
		/// Receives UTC seconds from url. Uses Unity's Awaitable for optimal async performance.
		/// </summary>
		/// <remarks>
		/// Available on Unity 2023.1+. Provides better performance than GetOnlineTimeTask
		/// by awaiting AsyncOperation directly without polling.
		/// Automatically switches to the current domain when running in WebGL to avoid CORS limitation.
		/// </remarks>
		/// <param name="url">Absolute url to receive time from.
		/// Make sure this server has proper Date values in the response headers
		...
		/// <param name="method">Method to use for url request. Use Head method if possible and fall back to get if server does not reply or block head requests.</param>
		/// <returns>OnlineTimeResult with UTC seconds or error.</returns>
		public static Awaitable<OnlineTimeResult> GetOnlineTimeAwaitable(string url, RequestMethod method = RequestMethod.Head)
		{
			return GetOnlineTimeAwaitable(url, System.Threading.CancellationToken.None, method);
		}
		...
		/// <param name="method">Method to use for url request. Use Head method if possible and fall back to get if server does not reply or block head requests.</param>
		/// <returns>OnlineTimeResult with UTC seconds or error.</returns>
		public static Awaitable<OnlineTimeResult> GetOnlineTimeAwaitable(string url, System.Threading.CancellationToken cancellationToken, RequestMethod method = RequestMethod.Head)
		{
			var uri = UrlToUri(url);
			return GetOnlineTimeAwaitable(uri, cancellationToken, method);
		}
		...
		/// <param name="method">Method to use for url request. Use Head method if possible and fall back to get if server does not reply or block head requests.</param>
		/// <returns>OnlineTimeResult with UTC seconds or error.</returns>
		public static Awaitable<OnlineTimeResult> GetOnlineTimeAwaitable(Uri uri, RequestMethod method = RequestMethod.Head)
		{
			return GetOnlineTimeAwaitable(uri, System.Threading.CancellationToken.None, method);
		}
		...
		/// <param name="method">Method to use for url request. Use Head method if possible and fall back to get if server does not reply or block head requests.</param>
		/// <returns>OnlineTimeResult with UTC seconds or error.</returns>
		public static async Awaitable<OnlineTimeResult> GetOnlineTimeAwaitable(Uri uri, System.Threading.CancellationToken cancellationToken, RequestMethod method = RequestMethod.Head)
		{
#if ACTK_WEBGL_BUILD
			EnsureCurrentDomainUsed(ref uri);
#endif
			...
			try
			{
				using (var wr = GetWebRequest(uri, method))
				{
					...
					// Directly await the AsyncOperation - no polling needed!
		...
		/// </summary>
		/// <remarks>
		/// Available on Unity 2023.1+. Provides better performance than ForceCheckTask
		/// by using Awaitable.NextFrameAsync() instead of Task.Delay.
		/// Detector events will be invoked too.
		/// </remarks>
		/// <example>
		/// <code>
		/// <![CDATA[
		/// MakeForcedCheckAsync();
		/// private async Awaitable MakeForcedCheckAsync()
		/// {
		///	    var result = await TimeCheatingDetector.Instance.ForceCheckAwaitable();
		///	    // check result
		///	    // ...
		/// }
		/// ]]>
		/// </code>
		/// </example>
		public async Awaitable<CheckResult> ForceCheckAwaitable()
		{
			if (!IsReadyForForceCheck())
				return LastResult;
