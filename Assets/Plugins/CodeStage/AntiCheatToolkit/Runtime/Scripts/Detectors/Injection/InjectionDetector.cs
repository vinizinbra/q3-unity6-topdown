...
namespace CodeStage.AntiCheat.Detectors
{
	...
	/// You can use detector completely from inspector without writing any code except the actual reaction on cheating.<br/><br/>
	/// Avoid using detectors from code at the Awake phase.<br/><br/>
	/// <strong>📝 Important Notes:</strong><br/>
	/// • Make sure you've checked the "Enable Injection Detector" option at the `Tools > Code Stage > Anti-Cheat Toolkit > Settings` window before using detector at runtime.<br/>
	/// • Always test detector on the target platform before releasing your application to the public.<br/>
	/// • It may detect some external assemblies as foreign, thus make sure you've added all external assemblies your application uses to the Whitelist (see section "Whitelist management" of the User Manual for details).<br/>
	/// • Disabled in Editor because of specific assemblies causing false positives. Use ACTK_INJECTION_DEBUG symbol to force it in Editor.<br/>
	/// • Only detects <strong>managed assembly injections</strong> (Mono/.NET assemblies). <strong>Native (unmanaged) code injections are NOT detected</strong> - they operate at a lower level and require additional protection mechanisms.<br/><br/>
	/// <strong>⚠️ Warning:</strong> Only Standalone and Android platforms with Mono scripting backend are supported.<br/>
	/// <example>
	/// <code>
	/// <![CDATA[
	...
	[HelpURL(ACTk.DocsRootUrl + "manual/detectors.html#injection-detector")]
	public class InjectionDetector : ACTkDetectorBase<InjectionDetector>
	{
		...
		/// </summary>
		/// <remarks>
		/// Provides information about what triggered the injection detection,
		/// typically the name of the injected assembly or the detection cause.
		/// </remarks>
		public InjectionDetectionInfo LastDetectionInfo => LastDetectionInfoAbstract as InjectionDetectionInfo;
		...
		private protected override bool IsUserListeningToCheatDetectedEvent()
		{
			return CheatDetected != null;
		}

		private protected override void InvokeCheatingDetectedEvent()
		{
		...
		private void TriggerCheatDetection(string reason)
		{
			...
			var detectionInfo = new InjectionDetectionInfo(reason);
