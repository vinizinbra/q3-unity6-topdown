namespace Quantum
{
    ...
    // Hero Mastery - Weapon Weight track (see docs/hero-mastery.md). Every hero has exactly one of
    // these lines (Weight fixed per hero, e.g. Max = Light) and one ElementMasteryData line - both
    ...
    public abstract unsafe partial class WeaponWeightMasteryData : PassiveUpgradeData
    {
        [Tooltip("Fixed per hero - which of the 3 weapon weight classes this Mastery line affects.")]
        ...
        public sealed override void Apply(Frame f, EntityRef entity, int rank)
        {
            ...
            f.AddOrGet<WeaponWeightMastery>(entity, out var mastery);
            mastery->Weight = Weight;
        ...
        // no bespoke component to install still compiles unchanged.
