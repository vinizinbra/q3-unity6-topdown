namespace Quantum
{
    ...
    // Too Angry to Die's live gating check, hooked into DamageUtility.ApplyDamage right where lethal
    // Health damage would otherwise clamp to 0 (see that call site). Generic mechanism - gated
    // purely by CheatDeathGuard's presence, never an "is this entity Max" check - but force-ending
    // "the current Overdrive activation" only makes sense for whoever actually has an active
    // HeroSkill, which today is only ever Max/Berserk (see TooAngryToDieSkillAction).
