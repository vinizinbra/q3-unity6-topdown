namespace Quantum
{
    ...
    // Targets whichever OTHER enemy (ally) within DetectionRange has the lowest CurrentHealth -
    // for a support-type enemy (e.g. a Healer) that wants to help whoever needs it most.
    public unsafe class LowestHealthAllyTargetingData : EnemyTargetingData
    {
        public override EntityRef SelectTarget(Frame f, EntityRef self)
        {
            ...
            for (int i = 0; i < hits.Count; i++)
            {
                ...
                if (best == EntityRef.None || health->CurrentHealth < bestHealth)
                {
                    best = candidate;
                    bestHealth = health->CurrentHealth;
                }
            }
