namespace Quantum
{
    ...
    // RuntimeConfig.HasteConfig.Duration - the same lingering window Zara's HasteOnHealUpgrade grants
    // via HasteEffectData - so "how long Haste lingers" is tuned in one place regardless of source,
    // rather than this aura fading near-instantly on a short local constant while the Speaker's much
    // sparser heal-pulse cadence leaves a multi-second tail. Shield Area Rate has no other source
    // granting it, so it keeps its own short AuraRefreshDuration instead.
    [Preserve]
    public unsafe class SentryAuraSystem : SystemMainThreadFilter<SentryAuraSystem.Filter>
    {
        ...
        // Falls back to the aura's own short constant if RuntimeConfig.HasteConfig can't resolve,
        ...
        private static FP ResolveHasteDuration(Frame f)
        {
            ...
            if (config == null)
            {
                Log.Error("[Sentry] Couldn't resolve RuntimeConfig.HasteConfig - Fire Rate aura falling back to its own short refresh duration");
