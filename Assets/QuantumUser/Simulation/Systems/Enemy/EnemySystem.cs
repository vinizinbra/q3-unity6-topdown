namespace Quantum
{
    ...
    public unsafe class EnemySystem : SystemMainThreadFilter<EnemySystem.Filter>, ISignalOnEnemyDied, ISignalOnEnemyKnockedBack, ISignalOnEntityPrototypeMaterialized
    {
        ...
        public void OnEntityPrototypeMaterialized(Frame f, EntityRef entity, EntityPrototypeRef prototypeRef)
        {
            ...
            {
                Log.Error($"[Enemy] {entity} has Enemy with no EnemyData assigned - Health/Shield stay at whatever the prefab authored");
                return;
            }
        ...
        // shared prototype can serve every enemy type/size - see EnemyDataAsset.Radius.
        private static void SeedRadius(Frame f, EntityRef entity, EnemyDataAsset data)
        {
            ...
            {
                Log.Error($"[Enemy] {entity} ({data.name}) has no PhysicsCollider3D - Radius {data.Radius} was not applied");
                return;
            }

            collider->Shape = Shape3D.CreateSphere(data.Radius);
            Log.Debug($"[Enemy] {entity} ({data.name}) collider radius seeded to {data.Radius}");
        }
        ...
        public override void Update(Frame f, ref Filter filter)
        {
            ...
            filter.PhysicsBody3D->GravityScale = data.Height.InitialState == EnemyHeightState.Flying ? FP._0 : FP._1;
        ...
        private static bool TickKnockbackRecovery(Frame f, ref Filter filter, EnemyDataAsset data)
        {
            ...
            if (filter.Enemy->Phase != EnemyActionPhase.Active &&
                data.Height.InitialState == EnemyHeightState.Grounded &&
        ...
        public void OnEnemyKnockedBack(Frame f, EntityRef entity)
        {
            ...
            Log.Debug($"[Knockback] {entity} staggered for {data.KnockbackRecoveryTime}s, Phase {enemy->Phase}");
        }
        ...
        private static void UpdateIdle(Frame f, ref Filter filter, EnemyDataAsset data)
        {
            ...
            if (target != EntityRef.None)
            {
                ...
                Log.Debug($"[Enemy] {filter.Entity} detected {target} within DetectionRange={data.DetectionRange}, switching Idle -> Chasing");
            }
        }
        ...
        {
            if (EnemyMovementUtility.TryFindNearestDecoy(f, filter.Transform3D->Position, data.DetectionRange, out EntityRef decoyTarget) == true)
                return decoyTarget;

            if (data.Targeting.IsValid == false)
                return EntityRef.None;

            return f.FindAsset(data.Targeting).SelectTarget(f, filter.Entity);
        }
        ...
        private static void UpdateChasing(Frame f, ref Filter filter, EnemyDataAsset data)
        {
            ...
            // retarget mid-attack.
            if (EnemyMovementUtility.TryFindNearestDecoy(f, selfPosition, data.DetectionRange, out EntityRef decoyTarget) == true && decoyTarget != filter.Enemy->Target)
            {
            ...
            if (sqrDistance > data.LeashRange * data.LeashRange)
            {
                Log.Debug($"[Enemy] {filter.Entity} target {filter.Enemy->Target} exceeded LeashRange={data.LeashRange} (sqrDistance={sqrDistance}), switching Chasing -> Idle");
                filter.Enemy->Target = EntityRef.None;
                filter.Enemy->Phase = EnemyActionPhase.Idle;
                return;
            }
            ...
            FP moveSpeed = data.MoveSpeed * StatusEffectUtility.GetSpeedMultiplier(f, filter.Entity);

            FPVector2 direction = data.Movement.IsValid == true
                ? f.FindAsset(data.Movement).ComputeMoveDirection(f, filter.Entity, filter.Enemy->Target)
                : default;
        ...
        private static void UpdateRecovery(Frame f, ref Filter filter, EnemyDataAsset data)
        {
            ...
            bool inLeashRange = sqrDistance <= data.LeashRange * data.LeashRange;
