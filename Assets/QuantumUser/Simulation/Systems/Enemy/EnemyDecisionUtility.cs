namespace Quantum
{
    ...
    public static unsafe class EnemyDecisionUtility
    {
            ...
            FPVector3 targetPosition, FP sqrDistanceToTarget, out EnemyActionData bestAction, out int bestSlot)
        {
            ...
            int slotCount = 1 + System.Math.Min(data.SkillActions.Count, MaxSkillActions);
        ...
        public static EnemyActionData ResolveAction(Frame f, EnemyDataAsset data, int slot)
        {
            if (slot <= 0)
            {
                return data.BasicAction.IsValid == true ? f.FindAsset(data.BasicAction) : null;
            }

            int skillIndex = slot - 1;

            if (skillIndex >= data.SkillActions.Count)
                return null;

            AssetRef<EnemyActionData> actionRef = data.SkillActions[skillIndex];
