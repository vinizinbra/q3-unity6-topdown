namespace Quantum
{
    ...
    public static unsafe class EnemyMovementUtility
    {
        ...
        {
            if (data.Height.InitialState == EnemyHeightState.Flying)
            {
                targetPosition.Y += data.Height.FlightHeight;
            }
        ...
        public static void MoveInDirection(Frame f, ref EnemySystem.Filter filter, EnemyDataAsset data, FPVector2 direction, FP speed)
        {
            ...
            bool isGrounded = data.Height.InitialState == EnemyHeightState.Grounded;
            ...
            if (currentlyGrounded == true && data.Height.CanJump == true && data.Height.CanCrossObstacles == true &&
                CanCrossLedge(f, filter.Transform3D->Position, flatDirection, data.Height.AnkleProbeHeight, data.Height.MaxLedgeHeight, data.Height.MantleProbeDistance, groundLayerMask) == true)
            {
                ...
                verticalVelocity = data.Height.JumpVelocity;
                filter.Enemy->JumpHorizontalVelocity = desiredVelocity;
            }
            else if (currentlyGrounded == true && data.Height.AvoidLedges == true &&
                HasGroundAhead(f, filter.Transform3D->Position, flatDirection, data.Height.EdgeProbeDistance, data.Height.EdgeCheckDistance, groundLayerMask) == false)
            {
                StopMovement(f, ref filter, data);
                return;
            }

            bool isFlying = data.Height.InitialState == EnemyHeightState.Flying;
        ...
        public static FP ComputeFlyingHoverVelocity(Frame f, ref EnemySystem.Filter filter, EnemyDataAsset data)
        {
            ...
            {
                filter.Enemy->FlyingHoverCheckTimer = data.Height.HoverCheckInterval;

                if (TryFindGroundHeight(f, filter.Transform3D->Position, GetGroundLayerMask(f), out FP groundY) == true)
                    filter.Enemy->FlyingHoverTargetHeight = groundY + data.Height.FlightHeight;
            }

            FP omega = data.Height.HoverSpringFrequency * FP.PiTimes2;
            ...
            FP force = -omega * omega * displacement - FP._2 * data.Height.HoverSpringDamping * omega * currentVelocityY;
        ...
        {
            filter.PhysicsBody3D->Velocity = data.Height.InitialState == EnemyHeightState.Flying
