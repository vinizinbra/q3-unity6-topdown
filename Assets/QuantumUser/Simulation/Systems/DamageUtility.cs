namespace Quantum
{
    ...
    // animation with an explosion (see FireEnemyExploded below). Only Heavy/Elite/Boss linger.
    public static unsafe class DamageUtility
    {
        ...
        private static FP ResolveFrontalDamageMultiplier(Frame f, EntityRef target, EntityRef owner)
        {
            ...
            if (enemyData.HasTrait(EnemyTrait.FrontalDamageReduction) == false)
                return FP._1;
            ...
            FP arcCos = FPMath.Cos(enemyData.FrontalDamageReductionArcDegrees * FP._0_50 * FP.Deg2Rad);

            if (dot < arcCos)
                return FP._1; // outside the frontal arc

            return FPMath.Clamp(FP._1 - enemyData.FrontalDamageReductionAmount, FP._0, FP._1);
        }
