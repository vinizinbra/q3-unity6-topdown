...
// Adrenaline's stack display: a single fixed icon (Adrenaline Rush's own, assigned once in the
// Inspector - PassiveData has no Icon field to read at runtime) plus a text for current Stacks and
// a "MAX" badge object that just shows/hides once Stacks reaches MaxStacks. Whole widget self-hides
// via root when Adrenaline isn't present (Adrenaline Rush not taken). By default self-binds to
// local slot 0 (player 1), same as SkillCooldownUiWidget - see autoBindLocalPlayerOne.
public class AdrenalineUiWidget : QuantumGlobalMonoBehaviour
{
    ...
    public override void QUpdate(QuantumGame game)
    {
        ...
        if (frame.Has<Adrenaline>(_entityRef) == false)
        {
            SetShown(false);
            return;
        }
        ...
        Adrenaline adrenaline = frame.Get<Adrenaline>(_entityRef);

        if (stacksText != null)
            stacksText.text = adrenaline.Stacks.ToString();

        if (maxStacksObject != null)
            maxStacksObject.SetActive(adrenaline.Stacks >= adrenaline.MaxStacks);
    }
